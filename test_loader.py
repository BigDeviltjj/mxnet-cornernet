...
roidb = filter_roidb(roidb)[:100]
#val_iter = DetRecordIter(roidb, cfg, cfg['batch_size'],False, num_workers = 1)
...
print('time of whole data: ', time.time() - s)
val_iter.reset()
s = time.time()
try:
    while True:
...
print('speed: ',i * val_iter.batch_size / duration)
import pdb
pdb.set_trace()
...
for it in val_iter:
    ...
    for img in imgs:
        ...
        cv2.imwrite("images/img+{}.jpg".format(j),img)
